...
def list_assignments(p):
    """Returns list of assignments assigned to teacher"""
...
def grade_assignment(p, incoming_payload):
    ...
    graded_assignment = Assignment.mark_grade(
        _id=grade_assignment_payload.id,  # Changed from ['id'] to .id
        grade=grade_assignment_payload.grade,  # Changed from ['grade'] to .grade
        auth_principal=p
    )
    db.session.commit()
